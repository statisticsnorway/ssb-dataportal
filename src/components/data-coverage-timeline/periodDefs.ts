 ...
 * Slot boundaries are plain date strings (`YYYY-MM-DD`); `Date.UTC` is used only
 * internally to calculate the last day of each slot's final month.
 *
 * Returns an empty array if `periodType` has no entry in `PERIOD_DEFS`, or if
 * `count` does not divide 12 evenly.
 *
 * @param year - The calendar year for which to generate slots.
 * @param periodType - The period type that defines slot count and labels.
 * @returns An array of `Slot` objects covering the full year.
 */
export const generateYearSlots = (year: number, periodType: PeriodFormat): Slot[] => {
  ...
  return Array.from({ length: def.count }, (_, i) => {
    const startMonth = i * monthsEach;
    const endMonth = (i + 1) * monthsEach;
